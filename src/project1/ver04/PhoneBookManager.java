...
public class PhoneBookManager {
	...
	public void dataInput(int choice) {
		...
		if(choice==1) {
			contacts[numOfContact++] = new PhoneInfo(iName, iPhone);
		}
		else if(choice==2) {
			System.out.print("전공: "); iMajor = sc.nextLine();
			System.out.print("학년: "); iGrade = sc.nextInt();
			contacts[numOfContact++] = new PhoneSchoolInfo(iName, iPhone, iMajor, iGrade);
		}
		else if(choice==3) {
			System.out.print("회사: "); iCompany = sc.nextLine();
			contacts[numOfContact++] = new PhoneCompanyInfo(iName, iPhone, iCompany);
		}
